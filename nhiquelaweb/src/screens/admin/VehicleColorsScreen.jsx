...
export default function VehicleColorsScreen() {
  ...
  const validateForm = () => {
    if (!formData.name.trim()) return 'O nome da cor � obrigatório.';
    if (!/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/.test(formData.hexCode)) return 'O código HEX � inválido (Ex: #FF0000).';
    if (!/^RGB\(\d{1,3},\d{1,3},\d{1,3}\)$/i.test(formData.rgbCode)) return 'O código RGB � inválido (Ex: RGB(255,0,0)).';
    return null;
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Tem a certeza que deseja eliminar esta cor? Esta ação � irreversível.')) {
      try {
    ...
    <div className="container-fluid py-4 animation-fade-in">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted mb-0">Faça a gestáo da paleta de cores global da plataforma Nhiquela.</p>
        </div>
