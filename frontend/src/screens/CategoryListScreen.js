...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case 'FETCH_SUCCESS':
      return { ...state, loading: false, categories: action.payload.categories,  pages: action.payload.pages};
...
export default function CategoryListScreen() {
  const [{ loading, error, categories, loadingDelete, successDelete, pages }, dispatch] =
    useReducer(reducer, {
      loading: true,
      error: '',
    });
 ...
 {[...Array(pages).keys()].map((x)=>(
     <Link className={x + 1 === Number(page)? 'btn text-bold': 'btn'} key={x+1} to={`/categoryList?page=${x+1}`}>
         {x+1}
     </Link>
 ))}
</div>
