...
async function debugSellersEndpoint() {
  ...
  console.log(`\n=== 🏪 Raw Sellers Count: ${sellers.length} ===`);
  sellers.forEach(s => {
    console.log(`User ID: ${s._id}, Name: ${s.name}, StoreName: ${s.seller?.name}, openstore: ${s.seller?.openstore}, storeStatus: ${s.seller?.storeStatus}`);
  });

  await mongoose.disconnect();
}

debugSellersEndpoint();
