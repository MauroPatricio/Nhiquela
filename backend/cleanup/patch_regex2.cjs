...
c = c.replace(/export const getWallet = async \(userId\) => \{\s*let wallet = await Wallet\.findOne\(\{ userId \}\);\s*if \(\!wallet\) \{\s*wallet = await Wallet\.create\(\{ userId, balance: 0 \}\);\s*\}\s*return wallet;\s*\};/g, 
...
c = c.replace(/export const hasSufficientBalance = async \(driverId\) => \{\s*const wallet = await getWallet\(driverId\);\s*const config = await getFinancialConfig\(\);\s*const limit = config\.allowNegativeBalance \? config\.creditLimit : config\.minOperationalBalance;\s*return wallet\.balance >= limit;\s*\};/g,
