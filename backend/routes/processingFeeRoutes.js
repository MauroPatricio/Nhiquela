...
router.get(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (genericFee) {
      ...
      res.status(404).json({ message: 'Taxa de processamento n�o encontrada para este servi�o' });
    }
  })
);
...
router.post(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (existing) {
      return res.status(400).json({ message: 'J� existe uma taxa configurada para este servi�o e estabelecimento.' });
    }
...
router.put(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (fee) {
      ...
      res.status(404).json({ message: 'Taxa de processamento n�o encontrada' });
    }
  })
);
