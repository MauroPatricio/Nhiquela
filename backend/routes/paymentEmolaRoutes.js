...
async function getAuthToken() {
  try {
    ...
    throw new Error('Falha na autentica��o com API Vitae');
  }
}

// Middleware para garantir token v�lido
async function ensureToken(req, res, next) {
  if (!bearerToken) {
    try {
      await getAuthToken();
    } catch (err) {
      return res.status(500).json({ error: 'Autentica��o falhou' });
    }
  }
  next();
}

// Enviar solicita��o de pagamento via Push USSD
paymentRouterEmola.post('/pay', ensureToken, expressAsyncHandler(async (req, res) => {
  const { phone, amount, orderId } = req.body;

  if (!phone || !amount || !orderId) {
    return res.status(400).json({ error: 'Campos obrigat�rios: phone, amount, orderId' });
  }
  ...
  try {
    ...
    res.status(200).json({
      message: 'Solicita��o de pagamento enviada com sucesso',
      data: response.data,
    });
  } catch (err) {
    console.error('Erro no pagamento Emola:', err.response?.data || err.message);
    res.status(500).json({ error: 'Erro ao enviar solicita��o Emola' });
  }
}));
...
// Webhook para confirma��o ass�ncrona do E-Mola
paymentRouterEmola.post('/webhook', expressAsyncHandler(async (req, res) => {
  ...
  // Deve ajustar as propriedades abaixo caso a documenta��o da Vitae use nomes diferentes.
  const { reference, transId, status } = req.body;

  if (!reference) {
    return res.status(400).send({ error: 'Refer�ncia n�o fornecida no webhook' });
  }
  ...
  try {
    ...
    if (!payment) {
      console.error('? Pagamento n�o encontrado para a refer�ncia:', reference);
      return res.status(404).send({ error: 'Pagamento n�o encontrado' });
    }

    // "SUCCESS" ou "0" � o c�digo comum para sucesso, depende da API E-Mola.
