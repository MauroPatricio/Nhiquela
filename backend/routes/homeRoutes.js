...
homeRouter.get(
  ...
  expressAsyncHandler(async (req, res) => {
    // 1️⃣ Categories with product counts
    ...
    // 2️⃣ Featured products (fallback to highest rated if no explicit flag)
    const featuredProducts = await Product.find({ isActive: true })
      .sort({ featured: -1, rating: -1 })
      .limit(8)
      .lean();

    // 3️⃣ Service count (only active services)
    ...
    // 5️⃣ Static promotional content (hard‑coded for now)
    const promos = {
