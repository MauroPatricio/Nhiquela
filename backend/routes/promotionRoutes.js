...
promotionRouter.get(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (promotion) {
      ...
      res.status(404).send({ message: 'Promo��o n�o encontrada' });
    }
  })
);
...
promotionRouter.post(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    res.status(201).send({ message: 'Promo��o criada com sucesso', promotion: createdPromotion });
  })
);
...
promotionRouter.put(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (promotion) {
      ...
      res.send({ message: 'Promo��o atualizada com sucesso', promotion: updatedPromotion });
    } else {
      res.status(404).send({ message: 'Promo��o n�o encontrada' });
    }
  })
...
promotionRouter.delete(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (promotion) {
      ...
      res.send({ message: 'Promo��o removida com sucesso' });
    } else {
      res.status(404).send({ message: 'Promo��o n�o encontrada' });
    }
  })
