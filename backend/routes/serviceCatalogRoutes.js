...
catalogRouter.post(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    res.status(201).send({ message: 'Servi�o criado', service: saved });
  })
);
...
catalogRouter.put(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (service) {
      ...
      res.send({ message: 'Servi�o atualizado', service: updated });
    } else {
      res.status(404).send({ message: 'Servi�o n�o encontrado' });
    }
  })
);
